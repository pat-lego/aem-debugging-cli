...
export default class ImsCommand extends BaseCommand<BaseEvent> {
    ...
    parse(): Command {
        ...
        program.command('generate:token')
            .description('Generates a bearer token from the developer console credentials file provided from the Adobe developer portal')
            ...
            .addOption(new Option('-m, --metascopes <metascopes>').conflicts('c').choices(['ent_cloudmgr_sdk', 'ent_aem_cloud_api']))
