...
export default class DispatcherCommand extends BaseCommand<BaseEvent> {
    ...
    parse(): Command {
        ...
        program.command('cache:activate')
            .alias('ca')
            .argument('<server>', 'The server location example: http://localhost:8080')
            .argument('<path>', 'The path to delete')
            .action((server: string, path: string) => {
                this.activateCache(server, path)
            })

        program.command('cache:deactivate')
            .alias('cd')
            .argument('<server>', 'The server location example: http://localhost:8080')
            .argument('<path>', 'The path to delete')
            .action((server: string, path: string) => {
                this.deactivateCache(server, path)
            })

        program.command('cache:flush')
            .alias('cf')
            .description('This is to be used for legacy dispatchers only')
            .argument('<server>', 'The server location example: http://localhost:8080')
            .argument('<path>', 'The path to delete')
            .action((server: string, path: string) => {
    ...
    async flushCache(server: string, path: string) {
        ...
        try {
            const response = await httpclient.get({ serverInfo: serverInfo, path: '/dispatcher/invalidate.cache', headers: { 'CQ-Action': 'FLUSH', 'CQ-Handle': path, 'CQ-Path': path, 'Content-Length': 0, 'Content-Type': 'application/octet-stream' } })
    ...
    async activateCache(server: string, path: string) {
        const serverInfo: ServerInfo = {
            auth: 'n/a',
            type: Authentication.NONE,
            serverUrl: server,
        ...
        try {
            const response = await httpclient.get({ serverInfo: serverInfo, path: '/dispatcher/invalidate.cache', headers: { 'CQ-Action': 'Activate', 'CQ-Handle': path, 'Content-Length': 0, 'Content-Type': 'application/octet-stream' } })
        ...
        try {
            const response = await httpclient.post({ serverInfo: serverInfo, path: '/dispatcher/invalidate.cache', headers: { 'CQ-Action': 'Activate', 'CQ-Handle': path, 'CQ-Path': path, 'Content-Length': 0, 'Content-Type': 'application/octet-stream' }, body: {} })
    ...
    async deactivateCache(server: string, path: string) {
        const serverInfo: ServerInfo = {
            auth: 'n/a',
            type: Authentication.NONE,
            serverUrl: server,
        ...
        try {
            const response = await httpclient.get({ serverInfo: serverInfo, path: '/dispatcher/invalidate.cache', headers: { 'CQ-Action': 'Deactivate', 'CQ-Handle': path, 'Content-Length': 0, 'Content-Type': 'application/octet-stream' } })
        ...
        try {
            const response = await httpclient.post({ serverInfo: serverInfo, path: '/dispatcher/invalidate.cache', headers: { 'CQ-Action': 'Deactivate', 'CQ-Handle': path, 'CQ-Path': path, 'Content-Length': 0, 'Content-Type': 'application/octet-stream' }, body: {} })
